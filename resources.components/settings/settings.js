...
/**
         ...
         */
        function switchElement(id, iconId) {
         ...
         *
         * @param {Element|null} errEl - The error element to update, if present.
...
/**
        ...
        function getDriverChoices(currentValue) {
            ...
            const source = Array.isArray(globalThis.DRIVERS) && globalThis.DRIVERS.length ? globalThis.DRIVERS : globalThis.FALLBACK_DRIVERS;
        ...
        async function updateDrivers(button, sectionId) {
            ...
            try {
                ...
                const driverKeys = Array.isArray(data)
                    ? data.map(manifest => manifest && manifest.key).filter(key => typeof key === 'string' && key)
                    : [];
...
/**
        ...
        function writeSectionStatus(sectionId) {
            ...
            return `
                    <div id="${clearString(sectionStatus.elementId)}"
                         class="field-hint section-status${clearString(statusClassName)}"
                         data-test-id="${clearString(sectionStatus.testId)}"
                         aria-live="polite">${clearString(statusMessage)}</div>`;
        }
         ...
         * - Wires the header to collapse/expand the body via switchElement().
        ...
        function writeSection({ id, title, desc, body, open = false }) {
            ...
            const descHtml = desc
                ? `<div class="section-desc">${clearString(desc)}</div>`
                : '';
            ...
            return `
            <div class="section">
                <div class="section-hdr" onclick="switchElement('sec-${id}','seci-${id}')">
         ...
         * @param {number} [options.min] - Lower bound for number fields.
         * @param {number} [options.max] - Upper bound for number fields.
         ...
         */
        function writeText({ path, label, hint, type = 'text', suffix, placeholder, min, max, required, maxLength, validate }) {
            // Resolve the current value and the optional hint/suffix/placeholder markup.
            const value = getPath(globalThis.STATE, path);
            const hintHtml = hint ? `<div class="field-hint">${clearString(hint)}</div>` : '';
            const suffixHtml = suffix ? `<span class="field-suffix">${clearString(suffix)}</span>` : '';
            const ph = placeholder ? ` placeholder="${clearString(placeholder)}"` : '';
            ...
            const wantsCheck = (required || validate) && kind !== 'number';
            const errId = 'fielderr-' + slug;
            const checkArg = wantsCheck
                ? `; updateFieldError('${errId}', this.value, ${!!required}, '${validate || ''}')`
                : '';
            const errHtml = wantsCheck
                ? `<div class="field-error" id="${errId}">${clearString(getFieldError(value, required, validate))}</div>`
                : '';

            // Build the base input element wired to setControlValue (+ optional validation).
            const inputHtml = `<input id="${inputId}" type="${type}" value="${clearString(value ?? '')}"${ph}${minAttr}${maxAttr}${maxLenAttr}
                    oninput="setControlValue('${path}', this.value, '${kind}'${minArg})${checkArg}" />`;
                ...
                ? `<div class="num-wrap">
                    ...
                    <div class="num-steppers">
                        <button type="button" class="num-step" tabindex="-1" aria-label="Increase"
                            onclick="stepNumber('${path}', '${inputId}', 1, ${min ?? 'null'}, ${max ?? 'null'})">${SVG_CARET_UP}</button>
                        <button type="button" class="num-step" tabindex="-1" aria-label="Decrease"
                            onclick="stepNumber('${path}', '${inputId}', -1, ${min ?? 'null'}, ${max ?? 'null'})">${SVG_CARET_DOWN}</button>
                    </div>
                </div>`
                : inputHtml;
            ...
            return `
            <div class="field">
                <label class="field-label">${clearString(label)}${star}${suffixHtml}</label>
                ${hintHtml}
                ${control}
                ${errHtml}
            </div>`;
            ...
            <div class="field">
                ...
                <div class="input-wrap">
                    <input type="text"
                           ...
                           oninput="setControlValue('sandbox', this.value)"
                           value="${clearString(value)}" />
                    <button type="button"
         ...
         * Behavior:
         * - Reads the current value from state at `path`.
         * - Wires the input to setControlValue and the button to switchSecret.
         *
         * @param {object} options - Control options.
         * @param {string} options.path - Dotted state path the control binds to.
         * @param {string} options.label - Human-friendly field label.
        ...
        function writeSecret({ path, label, hint }) {
            ...
            <div class="field">
                ...
                ${hintHtml}
                <div class="input-wrap">
                    <input id="${safeId}" type="password" value="${clearString(value ?? '')}"
                        oninput="setControlValue('${path}', this.value)" />
                    <button type="button" class="btn btn-ghost btn-sm"
                        onclick="switchSecret('${safeId}', this)">Show</button>
                </div>
            </div>`;
        }
        ...
        function writeSelect({ path, label, choices, hint }) {
            ...
            const optionsHtml = choices.map(choice => `
                <option value="${clearString(choice.value)}" ${choice.value === value ? 'selected' : ''}>
                    ${clearString(choice.text)}
                </option>`).join('');
            ...
            <div class="field">
                <label class="field-label">${clearString(label)}</label>
                ${hintHtml}
                <select onchange="setControlValue('${path}', this.value)">${optionsHtml}</select>
            </div>`;
        }
        ...
        function writeToggle({ path, label, hint }) {
            ...
            const value = !!getPath(globalThis.STATE, path);
            const hintHtml = hint ? `<div class="toggle-hint">${clearString(hint)}</div>` : '';
            ...
            <div class="field">
                <label class="toggle">
                    <input type="checkbox" ${value ? 'checked' : ''}
                        onchange="setControlValue('${path}', this.checked, 'bool')" />
                    <span class="switch"></span>
                    <span class="toggle-text">
                        <span class="toggle-label">${clearString(label)}</span>
                        ${hintHtml}
                    </span>
                </label>
            </div>`;
        ...
        function writeListLines({ path, label, hint }) {
            ...
            <div class="field">
                <label class="field-label">${clearString(label)}</label>
                ${hintHtml}
                <textarea oninput="setListValue('${path}', this.value)">${clearString(text)}</textarea>
            </div>`;
        }
        ...
        function writeJson({ path, label, hint, rows = 8 }) {
            ...
            const slug = path.replace(/[^a-z0-9]/gi, '-');
            const errId = 'jsonerr-' + slug;
            const taId = 'json-' + slug;
        ...
        function writeKeyValue({ path, label, hint, keyPlaceholder = 'Key', valuePlaceholder = 'Value', addLabel = 'Add', emptyText = 'None.' }) {
            ...
            const rows = entries.length
                ? entries.map(([key, val]) => `
                    <div class="kv-row">
                        <input type="text" class="kv-key" value="${clearString(key)}" placeholder="${clearString(keyPlaceholder)}"
                            onchange="renameEntryKey('${path}', '${clearString(key)}', this.value)" />
                        <input type="text" class="kv-val" value="${clearString(val)}" placeholder="${clearString(valuePlaceholder)}"
                            oninput="setEntryValue('${path}', '${clearString(key)}', this.value)" />
                        <button type="button" class="btn btn-ghost btn-sm" onclick="removeEntry('${path}', '${clearString(key)}')">Remove</button>
                    </div>`).join('')
                : `<div class="field-hint">${clearString(emptyText)}</div>`;
            ...
            <div class="field">
                ...
                <div class="add-row">
                    <button type="button" class="btn btn-ghost btn-sm" onclick="addEntry('${path}')">+ ${clearString(addLabel)}</button>
                </div>
            </div>`;
        }
        ...
        function writeDriverParameters(base, sectionId) {
            ...
            const driverOptions = getDriverChoices(driverValue)
                .map(choice => `<option value="${clearString(choice.value)}" ${choice.value === driverValue ? 'selected' : ''}>${clearString(choice.text)}</option>`)
                .join('');
            ...
            <div class="field-row">
                <div class="field">
                    ...
                    <div class="input-wrap">
                        <select onchange="setControlValue('${base}.driver', this.value)">${driverOptions}</select>
                        <button type="button"
                                class="btn btn-ghost btn-sm icon-btn"
                                title="Refresh Driver List"
                                data-test-id="${clearString(sectionId)}-refresh-driver-list-button"
                                aria-label="Refresh Driver List"
                                onclick="updateDrivers(this, '${clearString(sectionId)}')">${SVG_REFRESH}</button>
                    </div>
                </div>
                ${writeText({
...
/**
        ...
        function writeRepositoryCard(repo, index) {
            ...
            <div class="item-card">
                <div class="item-card-hdr">
                    <span class="item-card-title">${clearString(title)}</span>
        ...
        function writePluginsSection() {
            ...
            const repos = getPath(globalThis.STATE, 'settings.pluginsSettings.externalRepositories') || [];

            // Render one repository card per entry, or an empty-state note.
            const cards = repos.length
                ? repos.map((repo, index) => writeRepositoryCard(repo, index)).join('')
        ...
        function writeRecorderCard(recorder, index) {
            ...
            <div class="item-card">
                <div class="item-card-hdr">
                    <span class="item-card-title">${clearString(label)}</span>
                    <span class="spacer"></span>
        ...
        function writeMcpServerCard(name, server) {
            // Resolve the card's state base, the transport type, and the args text.
            ...
            const localFields = `
                <div class="field">
                    ...
                    <input type="text" value="${clearString(server?.command ?? '')}"
                        oninput="setControlValue('${base}.command', this.value)" />
                </div>
                <div class="field">
                    <label class="field-label">Arguments (One per Line)</label>
                    <textarea oninput="setListValue('${base}.args', this.value)">${clearString(args)}</textarea>
                </div>
                ${writeText({
            ...
            <div class="item-card">
                <div class="item-card-hdr">
                    <span class="item-card-title mono">${clearString(name)}</span>
                    <span class="spacer"></span>
                    <button type="button" class="btn btn-ghost btn-sm" onclick="removeServer('${clearString(name)}')">Remove</button>
                </div>
                <div class="field-row">
                    <div class="field">
                        <label class="field-label">Name</label>
                        <div class="field-hint">A short identifier for this tool server.</div>
                        <input type="text" value="${clearString(name)}"
                            onchange="renameServer('${clearString(name)}', this.value)" />
                    </div>
                    <div class="field">
                        <label class="field-label">Type</label>
                        <div class="field-hint">How G4 connects to the server.</div>
                        <select onchange="setServerType('${clearString(name)}', this.value)">
...
/**
         ...
         *
         * @param {string} path - The dotted state path that changed.
         * @param {*} rawValue - The raw value from the control.
         * @param {('text'|'number'|'bool')} [kind='text'] - How to coerce the value.
         * @param {number} [min=null] - Optional lower bound for number values.
         */
        function setControlValue(path, rawValue, kind = 'text', min = null) {
         ...
         *
         * @param {string} errId - Id of the error element.
         * @param {*} value - The current field value.
         * @param {boolean} required - Whether the field is required.
         * @param {string} rule - Extra validation rule (see getFieldError).
         */
        function updateFieldError(errId, value, required, rule) {
            // Resolve the error element and update its text with the latest message.
            const el = document.getElementById(errId);
            if (el) {
                el.textContent = getFieldError(value, required, rule);
            }
        }
         ...
         * @param {string} rawValue - The textarea contents.
         * @param {string} [errId] - Id of the element that shows parse errors.
         */
        function setJsonValue(path, rawValue, errId) {
         ...
         *
         * @param {string} taId - Id of the JSON textarea.
         * @param {string} path - The dotted state path the editor binds to.
         * @param {string} [errId] - Id of the element that shows parse errors.
         */
        function formatJson(taId, path, errId) {
         ...
         */
        function switchSecret(inputId, button) {
         ...
         *
         * @param {object|null} cred - The stored credentials value.
         * @returns {('none'|'basic'|'bearer')} The detected auth type.
         */
        function getCredentialsType(cred) {
            ...
            if (cred.type === 'basic' || cred.type === 'bearer') {
                return cred.type;
            }
        ...
        function addRepository() {
            ...
            const repos = Array.isArray(existing) ? existing : [];

            // Append a default repository, store it, re-render, and reveal the new card.
            repos.push({ name: '', url: '', version: 1, timeout: 300, capabilities: {}, headers: {} });
            setPath(globalThis.STATE, 'settings.pluginsSettings.externalRepositories', repos);
        ...
        function convertManifestPortsToText(value) {
            ...
            for (const [key, propertyValue] of Object.entries(value)) {
                const isPortValue = key === 'port' && propertyValue != null;
        ...
        function newManifest() {
            ...
            manifest.settings.automationSettings.maxParallel =
                Number.isFinite(parallel) && parallel >= 1 ? parallel : 1;
            ...
            const repos = manifest.settings?.pluginsSettings?.externalRepositories;
...
// Maps each section id to the function that builds its full markup, so a
         ...
         * field.
         *
         * @param {Element|null} el - The item element to reveal.
         */
        function showItem(el) {
            ...
            const field = el.querySelector('input[type=text], input[type=number], input[type=password], textarea, select');
            if (field) {
                field.focus();
            }
        }
